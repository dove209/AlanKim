...
export default function Scoring_15({ route, navigation }) {
    ...
    const onIncrease = () => {
        Vibration.vibrate(5)
        if (Q15.Q_score < maxRating) {
            setQ15({
                ...Q15,
                Q_score: Q15.Q_score + 1
            })
        }
    }
    //별점 -1
        ...
        if(Q15.Q_imges.length < 2){
            ...
            if (!image.cancelled) {
                const manipResult = await ImageManipulator.manipulateAsync(
                  ...
                  setQ15({
                      ...Q15,
                      Q_imges:[...Q15.Q_imges, manipResult.uri]
                  });
              }
        } else {
    ...
    const deleteImage = () => {
        Vibration.vibrate(5)
        if(Q15.Q_imges.length > 0) {
            setQ15({
                ...Q15,
                Q_imges: Q15.Q_imges.filter(image => image !== Q15.Q_imges[selectImageIdx - 1])
            });
        }
    ...
    let thumbnail = [];
    for (let i = 0; i < Q15.Q_imges.length; i++) {
        thumbnail.push(
            <TouchableOpacity key={i} onPress={() => setSelectImageIdx(i + 1)} style={{ marginLeft: 10, borderRadius: 10, borderWidth: 2, borderColor: selectImageIdx === (i + 1) ? '#00B2FF' : '#f9f9f9' }}>
                <Image style={styles.thumbnail} source={{ uri: Q15.Q_imges[i] }}>
                </Image>
            </TouchableOpacity>)
    }
    ...
    if (isEditComment) {
        return (
            <EditComment comment={Q15.Q_comment} cancelEditComment={cancelEditComment} submitEditComment={submitEditComment} changeComment={changeComment} />
        )
    } else {
        return (
                ...
                <View style={styles.optionsWrap}>
                    ...
                    <View>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Text style={styles.orderNum}>{`${Q15.Q_num}/16`}</Text>
                            <Text style={{ color: config.categoryColor[3], fontWeight: 'bold' }}>맛</Text>
                        </View>
                        <Text style={styles.questionsTitle}>{config.question[14]}</Text>
                        <Text style={styles.questionsContent}>{config.questionSub[14]}</Text>
                    </View>

                    {/* 별점 */}
                    <StarRating defaultRating={Q15.Q_score} maxRating={maxRating} onIncrease={onIncrease} onDecrease={onDecrease} updateRating={updateRating} />
